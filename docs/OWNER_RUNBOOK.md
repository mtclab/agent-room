...
7. Commit, push, and **tag**. This is the whole publish:
   ...
   (`v1.0.0-rc.3`) is published as a **pre-release** and does not move the
   image's `latest`.
...
holds nothing of yours:
    ...
    gh release download v1.0.0-rc.3
...
on a tag being pushed, so nothing happens until one is:
    ...
    git push origin :refs/tags/v1.0.0-rc.3 && git tag -d v1.0.0-rc.3
